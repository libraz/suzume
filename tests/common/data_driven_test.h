...
class TokenizationTestBase : public ::testing::TestWithParam<TestCase> {
 protected:
  ...
  void verifyMorphemes(const std::vector<core::Morpheme>& result, const std::vector<ExpectedMorpheme>& expected) {
    SCOPED_TRACE("Input: " + GetParam().input);

    // Check count
    ASSERT_EQ(result.size(), expected.size()) << "Morpheme count mismatch for: " << GetParam().input;
  ...
  void verifySurfaces(const std::vector<core::Morpheme>& result, const std::vector<ExpectedMorpheme>& expected) {
    SCOPED_TRACE("Input: " + GetParam().input);
    ...
    EXPECT_EQ(actual_surfaces, expected_surfaces) << "Surface mismatch for: " << GetParam().input;
  }
...
#define INSTANTIATE_TOKENIZATION_TEST_FROM_JSON(TestClass, SuiteName, JsonPath)                         \
  INSTANTIATE_TEST_SUITE_P(SuiteName, TestClass,                                                        \
                           ::testing::ValuesIn(suzume::test::JsonLoader::loadFromFile(JsonPath).cases), \
                           suzume::test::TestCaseNameGenerator())

// Macro to instantiate parameterized tests from JSON file with tag filter
#define INSTANTIATE_TOKENIZATION_TEST_FROM_JSON_WITH_TAG(TestClass, SuiteName, JsonPath, Tag)                      \
  INSTANTIATE_TEST_SUITE_P(SuiteName, TestClass,                                                                   \
                           ::testing::ValuesIn(suzume::test::JsonLoader::loadFromFile(JsonPath).filterByTag(Tag)), \
                           suzume::test::TestCaseNameGenerator())
