...
ConnectionRuleResult checkAdjStemToSugiruVerb(const core::LatticeEdge& prev,
  ...
  if (next.surface.size() < core::kTwoJapaneseCharBytes) return {};

  std::string_view first_two_bytes = next.surface.substr(0, core::kTwoJapaneseCharBytes);
  bool is_sugiru_garu = (first_two_bytes == "すぎ" || first_two_bytes == "がる" ||
                         first_two_bytes == "がり" || first_two_bytes == "がっ");
  if (!is_sugiru_garu) return {};
